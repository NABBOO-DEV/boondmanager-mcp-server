import { ReportingCompaniesSchema, ReportingProjectsSchema, ReportingResourcesSchema, ReportingSynthesisSchema, ReportingProductionPlansSchema, } from "../schemas/index.js";
...
export function registerReportingTools(server) {
    const reportingEndpoints = [
        ...
        },
        {
            ...
            datesRequired: false,
            filters: "reportingCategory, resourceTypes, resourceStates, period, resources/projects/contacts/companies, maxResources",
        },
        {
            name: "synthesis",
        ...
        },
        {
            ...
            datesRequired: true,
            filters: "resourceTypes, resourceStates, positioningStates, positioningPeriod, showContracts, projects/contacts/companies",
        },
    ];
    for (const ep of reportingEndpoints) {
        const datesNote = ep.datesRequired ? "\n⚠️ `startDate` + `endDate` (YYYY-MM-DD) sont REQUIS par l'API." : "";
        server.registerTool(`boond_reporting_${ep.name}`, {
            title: ep.title,
...
Returns: Données de reporting.`,
            ...
            return {
                content: [{ type: "text", text: formatListResponse(response, ep.entity) }],
            };
        });
    }
}
