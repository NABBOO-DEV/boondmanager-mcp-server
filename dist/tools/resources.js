import { ResourceCreateSchema, ResourceUpdateSchema, ResourceSearchSchema, ResourceTechnicalDataUpdateSchema, ReferenceCreateSchema, ReferenceUpdateSchema, ReferenceIdSchema, IdSchema, } from "../schemas/index.js";
import { registerSearchTool, registerGetTool, registerCreateTool, registerUpdateTool, registerDeleteTool, buildJsonApiBody, confirmDeletion, } from "./crud-factory.js";
...
export function mergeTechnicalData(current, input) {
    const out = {};
    const isBlank = (v) => v === undefined || v === null || v === "";
    // Scalars filled only when the existing value is blank.
    for (const key of ["title", "summary", "training"]) {
    ...
    for (const key of ["expertiseAreas", "activityAreas", "diplomas"]) {
        const incoming = input[key];
        if (Array.isArray(incoming)) {
            const cur = Array.isArray(current[key]) ? current[key].map((v) => String(v)) : [];
    ...
    if (Array.isArray(input.tools)) {
        const cur = Array.isArray(current.tools) ? current.tools : [];
        const existing = new Set(cur.map((t) => String(t.tool).toLowerCase()));
        const additions = input.tools.filter((t) => !existing.has(String(t.tool).toLowerCase()));
        out.tools = [...cur, ...additions];
    }
    if (Array.isArray(input.languages)) {
        const cur = Array.isArray(current.languages) ? current.languages : [];
        const existing = new Set(cur.map((l) => String(l.language).toLowerCase()));
        const additions = input.languages.filter((l) => !existing.has(String(l.language).toLowerCase()));
...
async function fetchTechnicalDataReferences(resourceId) {
    const response = await apiRequest(`/resources/${resourceId}/technical-data`);
    const entity = Array.isArray(response.data) ? response.data[0] : response.data;
    const refs = entity?.attributes?.references;
...
export function normalizeReferenceForApi(ref) {
    const out = {};
...
async function putTechnicalDataReferences(resourceId, references) {
...
export function registerResourceTools(server) {
    ...
    server.registerTool("boond_resources_technical_data_update", {
    ...
    server.registerTool("boond_resources_reference_create", {
        ...
        const provided = Object.fromEntries(Object.entries(attrs).filter(([, v]) => v !== undefined));
    ...
    server.registerTool("boond_resources_reference_update", {
    ...
    server.registerTool("boond_resources_reference_delete", {
