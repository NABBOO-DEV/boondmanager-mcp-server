...
const TimesheetCreateSchema = z
    ...
    .strict();
function formatTimesheetSummary(response) {
    ...
    const lines = data.map((item) => {
        ...
        const parts = [`[timesreport #${item.id}]`];
        if (attrs.startDate)
            parts.push(`Du: ${attrs.startDate}`);
        if (attrs.endDate)
            parts.push(`Au: ${attrs.endDate}`);
        if (attrs.state !== undefined)
            parts.push(`Statut: ${attrs.state}`);
        if (attrs.totalDays !== undefined)
            parts.push(`Jours: ${attrs.totalDays}`);
        if (attrs.totalHours !== undefined)
            parts.push(`Heures: ${attrs.totalHours}`);
        return parts.join(" | ");
    });
...
export function registerTimesheetTools(server) {
    server.registerTool("boond_timesheets_create", {
        ...
        return {
            content: [
                { type: "text", text: `✅ Feuille de temps créée avec succès.\nID: ${entity?.id}\n\n${text}` },
            ],
        };
    });
    // Get timesheets for a specific resource
    server.registerTool("boond_resources_timesheets", {
...
Returns: Liste des feuilles de temps de la ressource avec jours/heures et statut.`,
    ...
    server.registerTool("boond_timesheets_search", {
...
Returns: Liste des feuilles de temps correspondantes.`,
    ...
    server.registerTool("boond_timesheets_get", {
