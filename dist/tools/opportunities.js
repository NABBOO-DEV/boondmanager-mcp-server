 ...
 */
function buildOpportunityBody(params) {
    const { id, name, note, companyId, contactId, poleId, hrManagerId, mainManagerId, agencyId, ...attributes } = params;
...
export function registerOpportunityTools(server) {
    ...
    for (const tab of OPPORTUNITY_TABS) {
        server.registerTool(`boond_opportunities_${tab.name}`, {
            title: tab.title,
