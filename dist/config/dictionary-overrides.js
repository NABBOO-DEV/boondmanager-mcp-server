...
export const ACTION_ENTITIES = ["contact", "candidate", "resource", "opportunity", "project"];
...
function filterKnownEntities(section, raw, known, log) {
    const out = {};
    for (const [entity, labels] of Object.entries(raw)) {
        if (known.includes(entity)) {
            ...
            log.warn({ section, entity, known: [...known] }, `BOOND_DICTIONARY_OVERRIDES: unknown ${section} entity "${entity}" ignored`);
        }
    }
    return out;
}
 ...
 */
export function loadDictionaryOverrides(env = process.env) {
    ...
    if (!trimmed.startsWith("{")) {
        try {
            ...
            log.warn({ path: trimmed, err: err instanceof Error ? err.message : String(err) }, "BOOND_DICTIONARY_OVERRIDES: cannot read overrides file; custom labels disabled");
            return null;
        }
    }
    let parsed;
    try {
        ...
        log.warn({ err: err instanceof Error ? err.message : String(err) }, "BOOND_DICTIONARY_OVERRIDES: invalid JSON; custom labels disabled");
        return null;
    }
...
let cached = null;
let loaded = false;
export function getDictionaryOverrides() {
    if (!loaded) {
...
export function resetDictionaryOverridesForTests() {
    cached = null;
    loaded = false;
}
// --- Resolution helpers ---
...
function labelMapFor(section, entity) {
