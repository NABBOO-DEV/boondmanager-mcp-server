...
Retourne: JSON { isValid, rowCount, totalRows, rows } — rows = max 10 lignes de RÉSULTAT ; totalRows = nombre TOTAL de lignes de résultat (pas d'enregistrements). Si totalRows > rowCount, applique la règle de décision ci-dessus.`,
    ...
    async (params: { sql: string }) => {
      ...
      return { content: [{ type: "text" as const, text: truncate(JSON.stringify(payload, null, 2)) }] };
    }
  );

  // 5) Local schema reference (no network) --------------------------------------
  server.registerTool(
    "boond_extractbi_schema",
    {
...
Retourne: les tables/colonnes correspondantes.`,
    ...
    async (params: { table?: string; keywords?: string }) => {
      ...
      if (tableName) {
        ...
        for (const [col, info] of Object.entries(entry.c)) lines.push(`${col}: ${info}`);
      ...
      if (kw) {
        ...
        for (const [name, entry] of Object.entries(BOOND_DB_SCHEMA)) {
          ...
          const colHits = Object.entries(entry.c).filter(
            ([col, info]) => col.toLowerCase().includes(kw) || info.toLowerCase().includes(kw)
          );
          if (!tableHit && colHits.length === 0) continue;
          ...
          for (const [col, info] of shown) lines.push(`  ${col}: ${info}`);
        }
        const text =
      ...
      const list = Object.entries(BOOND_DB_SCHEMA).map(
        ([name, entry]) => `${name}${entry.d ? ` — ${entry.d.slice(0, 90)}` : ""}`
      );
